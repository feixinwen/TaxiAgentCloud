...
/**
 * 记录 Rag Service 的认证与授权拒绝事件，并委托 Spring Security 返回标准 401/403 响应。
 *
 * <p>拒绝日志不包含 Authorization 请求头或 Token 内容，避免凭证进入日志系统。</p>
 */
@Component
public class RagSecurityFailureHandler implements AuthenticationEntryPoint, AccessDeniedHandler {

    private static final Logger log = LoggerFactory.getLogger(RagSecurityFailureHandler.class);
    ...
    public void commence(
        ...
        log.warn(
                "event=rag_authentication_rejected traceId={} method={} path={} reason={}",
    ...
    public void handle(
        ...
        log.warn(
                "event=rag_access_denied traceId={} method={} path={} reason={}",
                response.getHeader(RequestTraceFilter.TRACE_ID_HEADER),
                request.getMethod(),
                request.getRequestURI(),
                exception.getClass().getSimpleName()
