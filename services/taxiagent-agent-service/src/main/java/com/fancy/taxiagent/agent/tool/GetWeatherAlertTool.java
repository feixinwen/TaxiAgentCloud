...
/**
 * 天气预警查询工具：解析经纬度参数并委托 {@link QweatherGateway#alert(String)}。
 */
@Service
public class GetWeatherAlertTool implements AgentTool {
     ...
     */
    public GetWeatherAlertTool(QweatherGateway gateway, ObjectMapper objectMapper) {
    ...
    @Override
    public String name() {
        return "getWeatherAlert";
    }

    @Override
    public String description() {
        return "查询指定经纬度当前生效中的天气预警（类型/级别/标题/描述）。参数：location 为\"经度,纬度\"字符串。";
    }
    ...
    public String execute(String jsonArgs, AgentExecutionContext context, AgentStreamSink sink) {
        try {
            ...
            return gateway.alert(QweatherGateway.round(arguments.location()));
