...
/**
 * Ticket Service 的 RSA 公钥加载与 JWT 解码配置。
 */
@Configuration(proxyBeanMethods = false)
@EnableConfigurationProperties(TicketJwtProperties.class)
public class TicketJwtConfiguration {

    @Bean
    @Lazy
    RSAPublicKey orderJwtPublicKey(TicketJwtProperties properties) {
        Resource resource = properties.getPublicKeyLocation();
        if (resource == null) {
            throw new BeanCreationException(
                    "TICKET_JWT_PUBLIC_KEY_LOCATION 未配置，Ticket Service 无法验证 Auth JWT"
            );
        }
        try (InputStream inputStream = resource.getInputStream()) {
            RSAPublicKey publicKey = RsaKeyConverters.x509().convert(inputStream);
            if (publicKey == null) {
                throw new BeanCreationException("Ticket Service JWT 公钥文件中没有可识别的 RSA 公钥");
            }
            return publicKey;
        } catch (IOException | IllegalArgumentException exception) {
            throw new BeanCreationException("Ticket Service JWT 公钥无法读取或格式错误", exception);
        }
    }

    @Bean
    @Lazy
    JwtDecoder orderJwtDecoder(RSAPublicKey orderJwtPublicKey, TicketJwtProperties properties) {
        String issuer = requireText(properties.getIssuer(), "TICKET_JWT_ISSUER");
        String accessAudience = requireText(properties.getAudience(), "TICKET_JWT_AUDIENCE");
        String serviceAudience = requireText(properties.getServiceAudience(), "TICKET_JWT_SERVICE_AUDIENCE");
    ...
    private OAuth2TokenValidatorResult validateTokenPurpose(
        ...
        return invalidToken("JWT token_type and audience do not match Ticket Service purpose");
    }
